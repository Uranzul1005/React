export function Skill() {
  return (
    <div className="flex flex-col items-center justify-center">
      <p className="mt-4 text-slate-500 dark:text-slate-300">
        The skills, tools and technologies I am really good at:
      </p>
      <div>
        <div className="grid pt-8 md:grid-rows-2 md:grid-cols-8 gap-6 grid-rows-6 grid-cols-3">
          <img src="/icons/JS.jpg" alt="javascript" />
          <img src="/icons/TS.jpg" alt="typescript" />
          <img src="/icons/React.jpg" alt="react" />
          <img src="/icons/Next.jpg" alt="next" />
          <img src="/icons/Node.jpg" alt="node" />
          <img src="/icons/Ex.jpg" alt="express" />
          <img src="/icons/Nest.jpg" alt="nest" />
          <img src="/icons/Socket.jpg" alt="socket" />
          <img src="/icons/SQL.jpg" alt="SQL" />
          <img src="/icons/Mongo.jpg" alt="mongo" />
          <img src="/icons/Sass.jpg" alt="sass" />
          <img src="/icons/Tailwind.jpg" alt="tailwind" />
          <img src="/icons/Figma.jpg" alt="figma" />
          <img src="/icons/Cy.jpg" alt="cy" />
          <img src="/icons/Story.jpg" alt="story" />
          <img src="/icons/Git.jpg" alt="git" />
        </div>
      </div>
    </div>
  );
}
